...
"""
def eval_prob_adaptive(unet, latent, text_embeds, scheduler, args, idx_map, node_map, child_nodes,  hier, remaining_prmpt_idxs, latent_size=64, all_noise=None, prompts_df=None):
        ...
        if stage == 0:
            while len(child_nodes)>1:
                ...
                for k,v in sorted_errors.items():
                    print(f"{k}: {prompts_df.class_name.iloc[k]}")
...
def main():
    ...
    parser.add_argument('--k', type=float, default=0.5, choices=(0.25, 0.4, 0.5, 2), help='Pruning Ratio')
    ...
    for i in pbar:
        ...
        pred_idx, pred_errors, topn = eval_prob_adaptive(unet, x0, text_embeddings, scheduler, args, idx_map, node_map, child_nodes, hier, remaining_prmpt_idxs, latent_size, all_noise, prompts_df)
